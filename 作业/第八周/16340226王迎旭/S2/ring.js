...
(function() 
{
  ...
  $('.apb').on('click', function onClickingButton() 
  {
    ...
    var $ordered = [];
    var length = $('#control-ring li.button').each(function() {$ordered.push($(this));}).length;
    function clickNext(index) 
    {
      ...
      $ordered[index].data('callback', function() 
      {
        $ordered[index].data('callback', false);
        clickNext(index + 1);
      });
      $ordered[index].trigger('click');
    }
    clickNext(0);
  });
  ...
  $('#control-ring li.button').on('click', function onClickingButton() {
    var $self = $(this);
    if (false === $('#button').data('hasStarted') || false === $self.hasClass('active') || true === $self.find('.unread').hasClass('show')) 
    {
       return;
    }
